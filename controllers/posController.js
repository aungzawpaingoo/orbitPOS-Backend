...
//   // Add this inside posController object
...
//       "SELECT total_amount, amount_paid, status FROM orders WHERE id = ? AND branch_id = ?",
//       [id, branchId]
//     );

//     if (orders.length === 0 || orders[0].status !== 'partially_paid') {
//       return res.status(400).json({ status: 'error', message: 'Order not found or not eligible for extra payment.' });
//     }

//     const order = orders[0];
//     const newAmount = parseFloat(order.amount_paid) + parseFloat(amountPaid);
//     const newStatus = newAmount >= parseFloat(order.total_amount) ? 'completed' : 'partially_paid';

//     await connection.query(
//       "UPDATE orders SET amount_paid = ?, status = ?, payment_type = ? WHERE id = ?",
//       [newAmount, newStatus, paymentMethod, id]
//     );

//     return res.status(200).json({ status: 'success', message: 'Balance updated' });
//   } catch (error) {
//     return res.status(500).json({ status: 'error', message: error.message });
//   } finally {
//     if (connection) connection.release();
...
const posController = {
  createTransaction: async (req, res) => {
    ...
    try {
      ...
      // 1. Stock Validation Loop (Updated to support both raw counts and object cart schemas)
      for (const product of products) {
        const cartItem = cart[product.id.toString()] || cart[product.id];
        const requestedQty = typeof cartItem === 'object' && cartItem !== null ? cartItem.quantity : cartItem;
      ...
      // 2. Insert Order Table Row with the final total passed from frontend
      const [orderResult] = await connection.query(
        "INSERT INTO orders (user_id, branch_id, total_amount, amount_paid, change_amount, status, created_at) VALUES (?, ?, ?, 0.00, 0.00, 'pending', NOW())",
        [userId, branchId, totalAmount]
      );
      ...
      // 3. Item Insertion Loop (Evaluates custom item prices mapped out by frontend)
      for (const product of products) {
        const cartItem = cart[product.id.toString()] || cart[product.id];
        
        let requestedQty = cartItem;
        let finalPriceAtSale = product.price; // Fallback to base product price table metric

        // Check if the cart structure contains an item object with custom sales modifications
        if (typeof cartItem === 'object' && cartItem !== null) {
          requestedQty = cartItem.quantity;
          if (cartItem.discountedPrice !== undefined) {
            finalPriceAtSale = cartItem.discountedPrice;
          }
        }

        await connection.query(
          'INSERT INTO order_items (order_id, product_id, quantity, price_at_sale) VALUES (?, ?, ?, ?)',
          [newOrderId, product.id, requestedQty, finalPriceAtSale]
        );
      }
