...
//       const [products] = await connection.query(
//         `SELECT p.id, p.name, p.price, p.conversion_factor, bi.stock 
//          FROM products p
//          JOIN branch_inventory bi ON p.id = bi.product_id
//          WHERE p.id IN (?) AND bi.branch_id = ? AND bi.organization_id = ?`,
//         [productIds, branchId, organizationId]
...
//       for (const product of products) {
//         const requestedQty = cart[product.id.toString()] || cart[product.id];
//         // Calculate required stock in base units based on conversion_factor
//         const requiredStock = requestedQty / product.conversion_factor;
...
//       const [items] = await connection.query(
//         `SELECT oi.product_id, oi.quantity, p.name, p.conversion_factor, bi.stock 
//          FROM order_items oi
//          JOIN products p ON oi.product_id = p.id
//          JOIN branch_inventory bi ON p.id = bi.product_id
//          WHERE oi.order_id = ? AND bi.branch_id = ? FOR UPDATE`,
...
//       for (const item of items) {
//         const deduction = item.quantity / item.conversion_factor;
...
//         await connection.query(
//           'UPDATE branch_inventory SET stock = stock - ? WHERE product_id = ? AND branch_id = ?',
//           [deduction, item.product_id, branchId]
//         );
//       }
...
//       const order = orderRows[0];
//       const [items] = await connection.query(
//         `SELECT oi.product_id, oi.quantity, p.conversion_factor 
//          FROM order_items oi 
//          JOIN products p ON oi.product_id = p.id 
//          WHERE oi.order_id = ?`, 
//         [id]
...
//       if (order.status === 'completed' || order.status === 'partially_paid') {
//         for (const item of items) {
//           const addition = item.quantity / item.conversion_factor;
//           await connection.query(
//             'UPDATE branch_inventory SET stock = stock + ? WHERE product_id = ? AND branch_id = ?', 
//             [addition, item.product_id, branchId]
//           );
//         }
//       }
