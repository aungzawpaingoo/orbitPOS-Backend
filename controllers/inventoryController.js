...
//         const finalSku = `${cleanName}-${uniqueString}`;
...
//             [organizationId, name, finalSku, price, imageUrl, category]
//         );
...
exports.getInventoryDashboard = async (req, res) => {
    ...
    try {
        ...
        const [metrics] = await pool.query(`
            SELECT 
                COUNT(DISTINCT p.sku) as totalProducts,
                IFNULL(SUM(bi.stock), 0) as totalStock,
                IFNULL(SUM(bi.stock * p.price), 0) as totalValue
            FROM branch_inventory bi
            JOIN products p ON bi.product_id = p.id
...
exports.addProduct = async (req, res) => {
    const { organizationId, branchId: userBranchId, role } = req.user;
    const { name, price, stock, targetBranchId, category, sku } = req.body;
    const connection = await pool.getConnection();
    ...
    try {
        ...
        if (sku && sku.trim() !== '') {
            ...
            const cleanName = name.replace(/[^a-zA-Z0-9]/g, '').toUpperCase().substring(0, 4);
            const uniqueString = Math.floor(1000 + Math.random() * 9000);
            finalSku = `${cleanName}-${uniqueString}`;
        }
       ...
       await connection.query(`
        ...
        console.error("====== CRITICAL ADD PRODUCT REJECTION LOG ======");
        console.error(error);
        console.error("================================================");
...
exports.updateProduct = async (req, res) => {
    ...
    try {
        ...
        // Custom SKU field update mapping evaluation block added here
        if (sku !== undefined) { 
            updateFields.push('sku = ?'); 
            updateParams.push(sku.toUpperCase().trim()); 
        }
        ...
        console.error("====== CRITICAL UPDATE PRODUCT REJECTION LOG ======");
        console.error(error);
        console.error("=================================================");
